...
k4CEDPickingHandler* k4CEDPickingHandler::instance = 0 ;
 
//hauke hoelbe
//k4CEDPickingMap k4CEDPickingHandler::map;
//k4CEDFunctionMap k4CEDPickingHandler::funcMap;
//k4CEDPickingHandler *k4CEDPickingHandler::instance = NULL;

std::vector<std::string> k4GaudiCED::_descs(CED_MAX_LAYER, ""); //layer descriptions

k4CEDPickingHandler& k4CEDPickingHandler::getInstance() {
  if( !k4CEDPickingHandler::instance){
        std::cout<<"new instance"<<std::endl;
	k4CEDPickingHandler::instance = new k4CEDPickingHandler();
    }
  return *k4CEDPickingHandler::instance;
}

// // void k4CEDPickingHandler::update(LCEvent *evt){
// //     const std::vector<std::string> *collNames = evt->getCollectionNames();
// //     const LCCollection *coll;
// //     const LCObject *obj;
// //     k4CEDFunctionMap::iterator iter;
...
// //     std::string typeName;
// //     std::string collName;
// //     k4CEDMapParticleObject particleObj;
...
// //                     particleObj.function=iter->second;
// //                 }else{
// //                     debug() << "k4CEDPickingHandler: cant register " << collName << "/" << typeName
// //                     << " (no function given)" << std::endl;
// //                     continue;
// //                 }
// //             }
// //             map.insert(std::pair<const int, k4CEDMapParticleObject>(obj->id(),particleObj));
// //         }
// //     }
// //     clock_t end = clock() ;
// //     debug() << "k4CEDPickingHandler::Map size: " << map.size() << " time: " << double( end - start ) / double(CLOCKS_PER_SEC) << "s" << std::endl;
// //}

// void k4CEDPickingHandler::printID(int id){
//     k4CEDPickingMap::iterator iter;
//     k4CEDMapParticleObject obj;
...
//     }else{
//       warning() << "No print function registered for this collection- or type name!" << endmsg;
//     }
...
// void k4CEDPickingHandler::registerFunction(std::string type, void (*printFunction)(const LCObject *)){
//     funcMap.insert(std::make_pair(type,printFunction));
// }

int k4CEDPickingHandler::kbhit(void) {
...
void k4GaudiCED::draw(const void* proc , int waitForKeyboard ) {
    int i=0;
    k4CEDPickingHandler &pHandler=k4CEDPickingHandler::getInstance();
    ...
    if( proc == instance()->_last ) {
        ...
        if ( waitForKeyboard == 1 ) {
            ...
            while(!k4CEDPickingHandler::kbhit()){
                //            while(!poll(pfd,1,0)){
...
void k4GaudiCED::drawDD4hepDetector( dd4hep::Detector& lcdd, bool _surfaces, StringVec _detailled){
  ...
  for( unsigned i=0,n=calorimeters.size() ; i<n ; ++i ){
    ...
    dd4hep::rec::LayeredCalorimeterData* calo = nullptr;
     info( ) << " ......processing " << detName << endmsg;  
    ...
    if(isDrawn){
      k4GaudiCED::set_layer_description( detName , layer );
       info( ) << detName << " has successfully been processed."<< endmsg;
    }
    std::cout<<std::endl;
  }
  //--- draw trackers 
  ...
  for( unsigned i=0,n=trackers.size() ; i<n ; ++i ){
    ...
    if (!isDrawn && visible){
      ...
      if(trkDisk){ 
        if (detailledDrawing(_detailled, detName)){
           info( )<<detName<<": Not drawn for now (appropriate geometry does not exist)"<<endmsg;
        }
        else{
  ...
  for( unsigned i=0,n=passiveDets.size() ; i<n ; ++i ){
    ...
    if (!isDrawn && visible){
      if (passiveConical){
         info( )<<detName<<" is not drawn for now (not needed)."<<endmsg;
...
bool DrawSurfaces(const dd4hep::rec::SurfaceManager &surfMan, std::string detName, unsigned color, int layer){
  ...
  if(sMap) {
    for (SMap::const_iterator it = sMap->begin(); it != sMap->end(); ++it){
      ...
      if (lines.empty()){
         info( )<<" **** drawSurfaces(): empty lines vector for surface "<< *surf <<endmsg;
        continue;
      }
  ...
  if (lineCounter > 0)
     info( )<<"Surfaces have been used."<<endmsg;
...
void getVisAttributes(dd4hep::DetElement det, unsigned &color, bool &visible) {
    ...
    if (thisVisAttribute.isValid()){
        ...
        if ( c != nullptr ){
         ...
         info()<<"color: pointer does not exist"<<endmsg;
